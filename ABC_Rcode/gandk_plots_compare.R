...
# Wasserstein SMC
filename = paste0(prefix, "gandkwsmc.n", nobservations,"outl",.5, ".RData")
load(filename)
results_was = results
...
g <- g + geom_label(data = data.frame(x = c(2.63, 2.73), y = c(3.9, 5.5), method = c("Wasserstein", "Posterior")),
...
g <- g + geom_label(data = data.frame(x = c(1.85, 1.53), y = c(1.8, 2.7), method = c("Wasserstein", "Posterior")),
                    aes(x = x, y = y, colour = method, label = method), size = 7) + theme(legend.position = "none")
...
g <- g + geom_label(data = data.frame(x = c(5.05, 4.4), y = c(0.6, 1.5), method = c("Wasserstein", "Posterior")),
                    aes(x = x, y = y, colour = method, label = method), size = 7) + theme(legend.position = "none")
...
g <- g + geom_label(data = data.frame(x = c(1.6, 1.3), y = c(3, 4.6), method = c("Wasserstein", "Posterior")),
                    aes(x = x, y = y, colour = method, label = method), size = 7) + theme(legend.position = "none")
