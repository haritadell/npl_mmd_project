...
def plot_posterior_marginals(B,thetas_wabc, thetas_npl_mmd, theta_star, n_cont, save_fig=False):
    ...
    fig, ax_array = plt.subplots(p, n_cont, gridspec_kw={'width_ratios': [3]*n_cont}, figsize=(13,13))
    ...
    if save_fig == True:
        fig.savefig('posterior_marginal_plot_gandk.png')
    fig.tight_layout()  
    return fig, ax_array
...
def plot_mse(thetas_wabc, thetas_npl_mmd, theta_star, n_cont, save_fig=False):
    ...
    if save_fig == True:
        fig.savefig('mse_plot_gandk.png')
    fig.tight_layout() 
    return fig, ax_array
