...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb  8 12:37:42 2022
...
class npl_prior():
    ...
    def __init__(self, X, B, m, p, l, model, a, T, model_name = 'gaussian'):
        self.model = model
        self.B = B  
        self.m = m  
        self.p = p  
    ...
    def draw_single_sample(self, weights, pseudo_samples):
        ...
        if self.model_name == 'toggle_switch':
            ...
            data = jnp.vstack((self.X,pseudo_samples))
            theta_j = self.minimise_MMD(data, weights)
    ...
    def draw_samples(self):
        ...
        param_vector = np.hstack((np.ones(self.n), (self.a/self.T)*np.ones(self.T)))   # set alpha as parameter
        weights = dirichlet.rvs(param_vector, size = self.B, random_state = 11)
        pseudo_samples = np.random.normal(loc=np.mean(self.X), scale=np.std(self.X), size=(self.B, self.T))  # Prior centering measure / change accordingly
        ...
        samples, wll_samples = vmap(self.draw_single_sample, in_axes=(0,0))(weights, pseudo_samples)
